...
SUBROUTINE wbsepp_decompose_eig_contributions()
  ...
  DO il1 = 1,pert%nloc
     ...
     DO iocc = 1,nbndx_occ
        DO iemp = 1,nbndx_emp
           IF(gamma_only) THEN
              anorm = 2._DP * DDOT(2*npw,dvg_exc(1,iocc,1,il1),1,evc(1,nbndx_occ+iemp),1)
              !
              IF(gstart == 2) THEN
                 anorm = anorm - REAL(dvg_exc(1,iocc,1,il1),KIND=DP) * REAL(evc(1,nbndx_occ+iemp),KIND=DP)
              ENDIF
           ELSE
           ...
           !
           projection_matrix(iocc,iemp,ig1) = SQRT(REAL(anorm,KIND=DP)**2 + AIMAG(anorm)**2)
        ENDDO
     ENDDO
     !
     DO ipol = 1,3
        ...
        DO iocc = 1,nbndx_occ
           IF(gamma_only) THEN
              anormr = anormr + 2._DP*DDOT(2*npw,dvg_exc(1,iocc,1,il1),1,d0psi(1,iocc,1,ipol),1 )
              IF(gstart == 2) THEN
                 anormr = anormr - REAL(dvg_exc(1,iocc,1,il1),KIND=DP) * REAL(d0psi(1,iocc,1,ipol),KIND=DP)
              ENDIF
           ELSE
        ...
        !
        os_strength(ipol,ig1) = anormr*anormr*2._DP/3._DP
        !
     ENDDO
     !
  ENDDO
  ...
  DO ig1 = 1,nvec
     !
     WRITE(stdout,"(/,5x,' #     Exciton : | ', i8,' |','   ','Excitation energy : | ', f12.6)") ig1, ev(ig1)
     ...
     !
     WRITE(stdout,"(/,10x,'     OCCUPIED STATES', 5x, 'UNOCCUPIED STATES', 5x, 'PROJECTION')")
     DO iocc = 1,nbndx_occ
        DO iemp = 1,nbndx_emp
           !
           ...
           !
           IF(temp >= 0.1_DP) THEN
              WRITE(stdout,"(16x, i8, 16x, i8, 14x, f12.6)") iocc, iemp, temp
           ENDIF
           !
        ENDDO
     ENDDO
  ...
  DO ig1 = 1,nvec
     ...
     DO iocc = 1,nbndx_occ
        DO iemp = 1,nbndx_emp
           WRITE(stdout,"(16x, i8, 16x, i8, 14x, f12.6)") iocc, iemp, projection_matrix(iocc,iemp,ig1)
        ENDDO
     ENDDO
     !
  ENDDO
  ...
  DO ig1 = 1,nvec
     DO i = 1,num_w
        ...
        IF(w_i <= w_end) THEN
           anormr = (os_strength(1,ig1)+ os_strength(2,ig1)+ os_strength(3,ig1))*ev(ig1)
           delta_func(i) = delta_func(i) + anormr*(epsil/((w_i-ev(ig1))*(w_i-ev(ig1)) + epsil*epsil))*(1._DP/pi)
        ENDIF
     ENDDO
  ENDDO
  !
