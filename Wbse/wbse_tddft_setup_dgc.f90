...
SUBROUTINE setup_dgc
  ...
  USE noncollin_module,     ONLY : noncolin, ux, nspin_gga, nspin_mag, domag
  ...
  !    add rho_core
  !
  fac = 1._DP / REAL (nspin_gga,KIND=DP)
  IF (noncolin.AND.domag) THEN
     ...
     ELSE
        ...
        IF (rh > epsr) THEN
           ...
           dvxc_rr (k, 1, 1) = e2 * (vrrxup + vrrcup + vrzcup * &
                (1._DP - zeta) / rh)
           dvxc_rr (k, 1, 2) = e2 * (vrrcup - vrzcup * (1._DP + zeta) / rh)
           dvxc_rr (k, 2, 1) = e2 * (vrrcdw + vrzcdw * (1._DP - zeta) / rh)
