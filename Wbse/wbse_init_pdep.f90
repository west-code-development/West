...
!
SUBROUTINE wbse_init_methods (ff_activate)
  !
  USE kinds,                ONLY : DP
  USE pwcom,                ONLY : isk,nks
  USE wavefunctions_module, ONLY : evc
  USE westcom,              ONLY : lrwfc,iuwfc,ev,dvg,n_pdep_eigen,npwq0x,&
                                   nbnd_occ
...
SUBROUTINE wbse_init_pdep_single_q (iks,ikq,xq,current_spin,nbndval,n_pdep_eigen,dvg,ev,l_restart_calc)
  ...
  USE westcom,              ONLY : wstat_dirname,sqvc,fftdriver,npwq0,npwq0x
  ...
  COMPLEX(DP), INTENT(IN) :: dvg(npwq0x, pert%nlocx)
  ...
  DO il1 = 1, bseparal%nlocx
     ...
     !
     ALLOCATE(phi_c(npwq0x),phi_x(npwq0x),psic_aux(dffts%nnr))
     !
     psic_aux(:) = (0.d0,0.d0)
     psic(:) = (0.d0,0.d0)
     !
     ...
     phi_x(:) = dvg(:,alnd) * sqvc(:)
     !
     CALL double_invfft_gamma(dffts,npwq0,npwq0x,phi_x,phi_c,psic_aux,TRIM(fftdriver))
     !
     IF (l_wannier_repr) THEN
        !
...
1111 CONTINUE
  ...
  !
  filename = TRIM( wstat_dirname )//"/PDEP_vc_iq"//TRIM(ADJUSTL(my_labeliq))//"_ik"//&
          TRIM(ADJUSTL(my_labelik))//"_spin"//TRIM(ADJUSTL(my_spin))//".dat"
