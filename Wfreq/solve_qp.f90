...
SUBROUTINE solve_qp_gamma_gpu(l_secant,l_generate_plot)
  ...
  REAL(DP),ALLOCATABLE :: d_body2_ifr_d(:,:,:)
#if defined(__CUDA)
  ATTRIBUTES(DEVICE) :: d_body2_ifr_d
#endif
  ...
  ALLOCATE(d_body2_ifr_d(n_lanczos,pert%nloc,ifr%nloc))
  !
  pert_nloc = pert%nloc
  ...
  DO iks = 1,kpt_pool%nloc ! KPOINT-SPIN
     ...
     DO ibloc = 1,band_group%nloc
        ...
        ! ------
        !
        !$acc parallel present(overlap,dtemp2,d_epsm1_ifr_trans)
        !$acc loop collapse(2)
        DO ifreq = 1,ifr_nloc
           DO im = 1,nbnd
              reduce_r = 0._DP
        ...
        ! -----
        !
        !$acc parallel present(overlap,ztemp2,z_epsm1_rfr_trans)
        !$acc loop collapse(2)
        DO ifreq = 1,rfr_nloc
           DO im = 1,nbnd
              reduce_c = 0._DP
        ...
        IF(l_enable_lanczos) THEN
           ...
           ENDDO
           !
           !$acc parallel present(braket,d_epsm1_ifr)
           !$acc loop collapse(3)
           DO ifreq = 1,ifr_nloc
              DO ip = 1,pert_nloc
                 DO il = 1,n_lanczos
...
SUBROUTINE solve_qp_k_gpu(l_secant,l_generate_plot)
  ...
  !$acc declare device_resident(overlap_loc)
  COMPLEX(DP), ALLOCATABLE :: z_body2_ifr_q_d(:,:,:)
#if defined(__CUDA)
  ATTRIBUTES(DEVICE) :: z_body2_ifr_q_d
#endif
  ...
  ALLOCATE(z_body2_ifr_q_d(n_lanczos,pert%nloc,ifr%nloc))
  !
  pert_nloc = pert%nloc
  pert_nglob = pert%nglob
  ifr_nloc = ifr%nloc
  ...
  DO iks = 1,k_grid%nps ! KPOINT-SPIN (MATRIX ELEMENT)
     ...
     DO ibloc = 1,band_group%nloc
        ...
        DO ikks = 1,k_grid%nps ! KPOINT-SPIN (INTEGRAL OVER K')
           ...
           ! ------
           !
           !$acc parallel present(overlap,ztemp2,z_epsm1_ifr_trans_q)
           !$acc loop collapse(2)
           DO ifreq = 1,ifr_nloc
              DO im = 1,nbnd
                 reduce = 0._DP
           ...
           ! -----
           !
           !$acc parallel present(overlap,ztemp2,z_epsm1_rfr_trans_q)
           !$acc loop collapse(2)
           DO ifreq = 1,rfr_nloc
              DO im = 1,nbnd
                 reduce = 0._DP
           ...
           IF(l_enable_lanczos) THEN
              ...
              ENDDO
              !
              !$acc parallel present(braket,z_epsm1_ifr_q)
              !$acc loop collapse(3)
              DO ifreq = 1,ifr_nloc
                 DO ip = 1,pert_nloc
                    DO il = 1,n_lanczos
