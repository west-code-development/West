...
#if defined(__CUDA)
  IF( gamma_only ) THEN
     CALL solve_qp_gamma_gpu( l_secant, l_generate_plot )
  ELSE
     CALL solve_qp_k_gpu( l_secant, l_generate_plot )
  ENDIF
#else
  IF( gamma_only ) THEN
...
#if defined(__CUDA)
!-----------------------------------------------------------------------
SUBROUTINE solve_qp_gamma_gpu(l_secant,l_generate_plot)
  !-----------------------------------------------------------------------
  !
  ! ... This subroutine solves the DBS problem for GAMMA, at non-zero freqeuncies.
  ! ... Perturbations are distributed according to the POT mpi_communicator
  ...
  USE wfreq_db,             ONLY : wfreq_db_write
  USE types_bz_grid,        ONLY : k_grid
  USE west_gpu,             ONLY : dtemp_d,ztemp_d,d_epsm1_ifr_d,d_epsm1_ifr_trans_d,z_epsm1_rfr_trans_d,&
                                 & d_body2_ifr_d,allocate_qp_gpu,deallocate_qp_gpu
  !
  IMPLICIT NONE
  !
  ! I/O
  ...
  INTEGER :: notconv
  REAL(DP),ALLOCATABLE :: dtemp(:)
  REAL(DP),PINNED,ALLOCATABLE :: dtemp2(:,:)
  COMPLEX(DP),PINNED,ALLOCATABLE :: ztemp2(:,:)
  REAL(DP),ALLOCATABLE :: diago(:,:)
  ...
  DO iks = 1,kpt_pool%nloc ! KPOINT-SPIN
     ...
     DO ibloc = 1,band_group%nloc
           ...
           DO im = 1,nbnd
              ...
              DO glob_jp = 1,n_pdep_eigen_to_use
                 DO ip = 1,pert_nloc
                    ...
                    & *d_epsm1_ifr_trans_d(ip,glob_jp,ifreq)
                 ENDDO
              ENDDO
              dtemp_d(im,ifreq) = reduce_r
           ENDDO
        ENDDO ! ifreq
        !$acc end parallel
        !
           ...
           DO im = 1,nbnd
              ...
              DO glob_jp = 1,n_pdep_eigen_to_use
                 DO ip = 1,pert_nloc
                    ...
                    & *z_epsm1_rfr_trans_d(ip,glob_jp,ifreq)
                 ENDDO
              ENDDO
              ztemp_d(im,ifreq) = reduce_c
           ENDDO
        ENDDO ! ifreq
        !$acc end parallel
        !
        ...
        IF(l_enable_lanczos) THEN
           ...
           !$acc end parallel
           !
           d_body2_ifr(:,:,:,ib,iks_g) = d_body2_ifr_d
           !
        ENDIF
        !
        CALL update_bar_type(barra,'coll_gw',1)
...
!
!-----------------------------------------------------------------------
SUBROUTINE solve_qp_k_gpu(l_secant,l_generate_plot)
  !-----------------------------------------------------------------------
  !
  ! ... This subroutine solves the DBS problem for GAMMA, at non-zero freqeuncies.
  ! ... Perturbations are distributed according to the POT mpi_communicator
  ...
  USE wfreq_db,             ONLY : wfreq_db_write
  USE types_bz_grid,        ONLY : k_grid,q_grid
  USE west_gpu,             ONLY : ztemp_d,z_epsm1_ifr_q_d,z_epsm1_ifr_trans_q_d,z_epsm1_rfr_trans_q_d,&
                                 & z_body2_ifr_q_d,allocate_qp_gpu,deallocate_qp_gpu
  !
  IMPLICIT NONE
  !
  ! I/O
  ...
  DO iks = 1,k_grid%nps ! KPOINT-SPIN (MATRIX ELEMENT)
     ...
     DO ibloc = 1,band_group%nloc
        ...
        DO ikks = 1,k_grid%nps ! KPOINT-SPIN (INTEGRAL OVER K')
              ...
              DO im = 1,nbnd
                 ...
                 DO glob_jp = 1,n_pdep_eigen_to_use
                    DO ip = 1,pert_nloc
                       ...
                       & *z_epsm1_ifr_trans_q_d(ip,glob_jp,ifreq,iq)
                    ENDDO
                 ENDDO
                 ztemp_d(im,ifreq) = reduce
              ENDDO
           ENDDO ! ifreq
           !$acc end parallel
           !
           ztemp2(:,1:ifr%nloc) = ztemp_d(:,1:ifr%nloc)
           !
           ...
           DO ifreq = 1,rfr_nloc
              DO im = 1,nbnd
                 ...
                 DO glob_jp = 1,n_pdep_eigen_to_use
                    DO ip = 1,pert_nloc
                       reduce = reduce+CONJG(overlap(glob_jp,im))*overlap_loc(ip,im) &
                       & *z_epsm1_rfr_trans_q_d(ip,glob_jp,ifreq,iq)
                    ENDDO
                 ENDDO
                 ztemp_d(im,ifreq) = reduce
              ENDDO
           ...
           !$acc end parallel
           !
           ztemp2(:,1:rfr%nloc) = ztemp_d(:,1:rfr%nloc)
           !
           ...
           IF(l_enable_lanczos) THEN
              ...
              !$acc end parallel
              !
              z_body2_ifr_q(:,:,:,ib,iks,iq) = z_body2_ifr_q_d
              !
           ENDIF
           !
           CALL update_bar_type(barra,'coll_gw',1)
