...
MODULE lanzcos_restart
  ...
  CONTAINS
    ...
    SUBROUTINE lanzcos_restart_write (nipol_input, pliter_stop, lriter_stop)
      ...
      USE westcom,              ONLY : west_prefix, wstat_dirname 
      ...
      !
      dirname = TRIM( wstat_dirname ) // '/' // TRIM("summary.xml")
      IF ( mpime == root ) THEN
         !
         ! ... open XML descriptor
         !
         CALL iotk_free_unit( iunout, ierr )
         CALL iotk_open_write( iunout, FILE = TRIM( wstat_dirname ) // '/' // TRIM("summary.xml") , BINARY = .FALSE., IERR = ierr )
         !
      END IF
      !
    ...
    SUBROUTINE lanzcos_restart_read (nipol_input, pliter_stop, lriter_stop)
      ...
      IF ( mpime==root ) THEN
         ...
         CALL iotk_open_read( iun, FILE = TRIM( wstat_dirname ) // '/' // TRIM( 'summary.xml' ), IERR = ierr )
      ENDIF
      !
      CALL mp_bcast( ierr, root, world_comm )
      !
